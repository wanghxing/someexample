	...
	**/
	...
	PickerHandler.prototype._relocate = function(){
		...
		if ($input != null && $picker.data('isInline') === false) { // Float mode
			...
			$picker.parent().css('top', ($input.offset().top + input_outer_height+5) + 'px');

			// Display left side stick to input
			var iInputLen = $input.width(),
				iInputPadLe = parseInt($input.css('padding-left').split("px")[0]),
				iInputPadRi = parseInt($input.css('padding-right').split("px")[0]),
				iPicLen = $picker.width(),
				iDif = iPicLen-iInputLen-iInputPadLe-iInputPadRi;
			$picker.parent().css('left', $input.offset().left-iDif + 'px'); 
		...
		for (var zz = 0; i < cellNum; i++) {
			...
			if (
				...
				$td.click(function() {
					...
					if ($picker.data("dateOnly") === true && $picker.data("isInline") === false){
						// Close a picker
						ActivePickerId = -1;
						$picker.hide();
					}
...
//			$timelist.css("height", Math.max($calendar.innerHeight() - 10, 200) + 'px');
				...
				if (
					...
					$o.click(function() {
						...
						if ($picker.data("isInline") === false){
							// Close a picker
							ActivePickerId = -1;
							if(!options.isStopClick){  // ! 2017.08.03 wanghaoxing改动此处 加了个“！”
								$picker.hide();
							}
	...
	var init = function($obj, opt) {
		...
		if(opt.dateOnly){
			...
			$calendar.css({"float":"none","margin-left":"0px","margin-bottom":"0px","border-right":"0px"}); 
		}else{
			$calendar.addClass('datepicker_calendar');
		}
		var $table = $('<table>');
	...
	$('body').click(function(e){
		if(options.isStopClick){
			var sName = $(e.target).attr("sDateName");
			if($(e.target).attr("title")=="Previous month"||$(e.target).attr("title")=="Next month"){
				return;
			}
